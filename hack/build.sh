...
# Examples:
#   hack/build.sh multicluster-provider-fake
#   BUILD_PLATFORMS=linux/amd64,linux/arm64 hack/build.sh multicluster-provider-fake
...
function build_binary_for_platform() {
  ...
  local gopkg="${PROVIDER_GO_PACKAGE}/$(util::get_target_source $target)"
  set -x
